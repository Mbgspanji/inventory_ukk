...
<head>
    <style>
        ...
        .total-section { margin-top: 10px; border-top: 1px solid #000; }
        
        /* CSS untuk Tanda Tangan */
        .signature-wrapper { margin-top: 20px; width: 100%; }
...
<body>
    ...
    <table>
        <tr><td width="40%">No. Transaksi</td><td>: #LND-{{ $lending->id }}</td></tr>
        <tr><td>Peminjam</td><td>: {{ $lending->borrower_name }}</td></tr>
        <tr><td>Tgl Pinjam</td><td>: {{ \Carbon\Carbon::parse($lending->date)->format('d/m/Y H:i') }}</td></tr>
        @if($lending->returned_date)
        <tr><td>Tgl Kembali</td><td>: {{ \Carbon\Carbon::parse($lending->returned_date)->format('d/m/Y H:i') }}</td></tr>
        @endif
    </table>
